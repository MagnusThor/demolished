...
float map(vec3 p) {
	vec3 n = vec3(0, 1,  (freqs[0]-0.75));
	float k1 = 3.0 * freqs[3];
	float k2 = (sin(p.x * k1) + sin(p.z * k1)) * (0.2) + freqs[2];
...
void main() {
	...
	dir.xz = rot(dir.xz, time * (freqs[3] * 0.0125));
	dir = dir.yzx;
	dir.xz = rot(dir.xz, time * (freqs[0] * 0.025));
	dir = dir.yzx;

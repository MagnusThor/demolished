...
// Space-ship travel experiment for demoished
...
vec4 formula(vec4 p) {
    p.xz = abs(p.xz + 1.) - abs(p.xz - 1.) - p.xz; //  <---- Look at this
...
float screen(vec3 p) {
    ...
    return min(max(d1, abs(p.x - .3) - .01), max(d2, abs(p.x + 2.3) - .01)); // <---- Look at this
}
...
Ambient occlusion
*/
float calcAO(const vec3 pos,
    const vec3 nor) {
    float aodet = detail * 20.;
    float totao = 0.0;
    float sca = 13.0;

...
float map(vec3 p){
    ...
    vec2 tun = abs(p.xy - path(p.z)) * vec2(.5,0.7071);
    tun = pow(tun,vec2(4.));
    float n = 1. -pow(tun.x + tun.y, 1./4.) + (0.5-surfFunc(p));
    return min(n,p.y + FH);

    // vec2 tun = abs(p.xy - path(p.z)) * vec2(.5,.7071);
    // float n = 1. -length(tun) - (.5-surfFunc(vec3(p.x,p.y,p.z)));
    // return min(n,p.y + FH);

    // vec2 tun = (p.xy - path(p.z)) * vec2(.5,.7071);
    // float n = 1. - length(tun) + (.5-surfFunc(p));
    // return min(n,p.y + FH);
...
// continuous Laplace differential operator? Either way, it gives you a sr 
...
void main(){
    ...
    // Light positioning. One is a little behind the camera, and the other is further down the tunnel.
 	vec3 light_pos = camPos + vec3(freqs[0] * 0.5, 0.125, -0.125);// Put it a bit in front of the camera.
    ...
    // light_pos2.xy = rot(light_pos2.xy, time *  0.0125);
	// light_pos.xy += rot(light_pos.xy,time * 0.0125);
    

    // lookAt.xz = rot(lookAt.xz,time * 0.0125);
    ...
    float col = 3.0 - (3.*length(2.*uv));	
	vec3 coord = vec3(atan(lookAt.x,lookAt.y)/6.2832+.5, length(lookAt)* (freqs[3])*0.3, .5);
	
	for(int i = 1; i <= 7; i++){
		float power = pow(3.0, float(i));
        col  += (2.5 / power) * noise(coord + vec3(0.,-time*.05, time*.01), power*8.);
	}
	vec4 tunnel =vec4(clamp(sceneCol, 0., 1.), 1.0);
	//  vec4 smoke = vec4( pow(max(col,0.),2.)*2.4, pow(max(col,0.),2.)*1.0, pow(max(col,0.),2.)*0.4 , 1.0);
